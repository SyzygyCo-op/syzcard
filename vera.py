...
class Action(object):
    ...
    def parse(vera, s):
        ...
        if s["service"] == "urn:micasaverde-com:serviceId:DoorLock1":
            return LockAction.parse(vera, s)
...
class LockAction(Action):
    """
    Action which operates against a standard lock which has on/off semantics.
    """

    def __init__(self, device=None, value=None):
        """
        Creates a LockAction object.
        :param device: Device object describing the device to apply
        :param value: boolean value for lock
        """
        self.device = device
        self.value = value
    ...
    def parse(vera, s):
        """
        Converts LUUP SetTarget values to a LockAction object.
...
class Device(object):
    ...
    def get_lock(self):
        """
        Get the current state of a lock device.  Returns a boolean value.
        """
    ...
    def set_lock(self, value):
        """
        Changes the setting of a lock device.
